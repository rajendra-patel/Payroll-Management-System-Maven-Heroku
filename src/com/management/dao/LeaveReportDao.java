...
//			repo.getConfig().prepareStatement(psmtquerry);
			...
			System.out.println("inside leave dao id" + emp_id);
			int id = 0;
			/*
			 * querry =
			...
			while (rs.next()) {
				...
				System.out.println("all leaves " + leaves.get(id).toString());
			}
...
//				"select * from leaves where Leave_Id=" + String.valueOf(id);
	...
	public Leave postLeaveDao(int emp_id, Leave leave) {
		...
		try {
			...
			if (id == emp_id) {
				...
				repo.executeUpdateRep("INSERT INTO track_leaves(Emp_Id, Total_Leaves, Track_Left) VALUES("+emp_id+", "+leave.getLeave_total()+", "+leave.getLeave_total()+")");
				}
	...
	public Leave putLeaveDao(int emp_id, int leave_id, Leave leave) {
		...
		try {
			...
			rs = repo.executeQuerryRep("select Leave_Employee_Id, Leave_Id, Leave_Type, Leave_From, Leave_Till from leaves where Leave_Id="+leave_id+" and Leave_Employee_Id="+lID);
			if (rs.next()) {
				this.leave = new Leave();
			} else {
				this.leave = null;
			...
			if (leave.getLeave_till() != null) {
				...
				rs.updateRow();
				ResultSet ret = repo.executeQuerryRep("Select * from leaves where Leave_Employee_Id="+emp_id+" and Leave_Id="+leave_id);
				while(ret.next()) {
				...
				repo.executeUpdateRep("update track_leaves t1 join leaves t2 on t1.Emp_Id = t2.Leave_Employee_Id set t1.Track_Left = t2.Leaves_Left where t1.Emp_Id="+emp_id+" and t2.Leave_Id="+leave_id);
				ret.close();
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
	...
	public boolean deleteLeaveDao(int emp_id, int leave_id) {
		ResultSet rs = null;
		System.out.println(emp_id + " " + leave_id);
		boolean rid = false;
		...
		try {
			...
			rs.absolute(1);
			rs.deleteRow();
			int dRa = repo.executeUpdateRep("update track_leaves l1 inner join leaves l2 set l1.Track_Left=l2.Tracked_Leaves where l1.Emp_Id="+emp_id);
			if (dRa==0) {
